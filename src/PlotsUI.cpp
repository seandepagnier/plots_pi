...
PlotsDialogBase::PlotsDialogBase( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxDialog( parent, id, title, pos, size, style )
{
	...
	// Connect Events
	this->Connect( wxEVT_CLOSE_WINDOW, wxCloseEventHandler( PlotsDialogBase::OnClose ) );
	this->Connect( wxEVT_SIZE, wxSizeEventHandler( PlotsDialogBase::OnSize ) );
	m_swPlots->Connect( wxEVT_CHAR, wxKeyEventHandler( PlotsDialogBase::Relay ), NULL, this );
	...
	m_menu1->Bind(wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( PlotsDialogBase::OnConfiguration ), this, m_mConfiguration->GetId());
}

PlotsDialogBase::~PlotsDialogBase()
{
	// Disconnect Events
	this->Disconnect( wxEVT_CLOSE_WINDOW, wxCloseEventHandler( PlotsDialogBase::OnClose ) );
	this->Disconnect( wxEVT_SIZE, wxSizeEventHandler( PlotsDialogBase::OnSize ) );
	m_swPlots->Disconnect( wxEVT_CHAR, wxKeyEventHandler( PlotsDialogBase::Relay ), NULL, this );
